...
class graph:
    ...
    def computeScores(self):
        ...
        j = 1
        while j < self.m + 1:
            nodes[0][j].weight = nodes[0][j-1].weight 
        ...
        while i < self.n + 1:
            ...
            while j < self.m + 1:
                ...
                if nodes[i-1][j].left != nodes[i][j-1].down: 
                    diag = nodes[i-1][j-1].weight - 2
                    
                up = nodes[i-1][j].weight - 2
                left = nodes[i][j-1].weight - 2
    ...
    def align(self, s1, s2):
        ...
        while i != 0 and j != 0 and backtrack[i][j] != None:
            #print i, j
            #print backtrack[i][j]
...
class Overlap:
    ...
    def __init__(self):
        ...
        Constructor
        '''
        self.dataSet = "dataset_77_7 (1).txt"
        #self.testSet = "lcs.txt"
        self.testSet = "overlap_alignment.txt"
